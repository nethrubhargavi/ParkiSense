...
function FamilyHistory({ patientId, onSaveSuccess }) {
  ...
  const [currentMember, setCurrentMember] = useState({ relation: '', ageAtDiagnosis: '' })
  ...
  const handleAddFamilyMember = () => {
    ...
    setFamilyMembers([...familyMembers, { id: Date.now(), relation: currentMember.relation, relationLabel, ageAtDiagnosis: age }])
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const response = await apiFetch(`${API_BASE}/family-history`, {
        method: 'POST',
        ...
        <section className="form-section">
          ...
          <div className="family-question">
            ...
            <div className="yes-no-buttons">
              <button type="button" onClick={() => { setHasFamilyHistory(true); setError('') }}
                className={`yes-no-btn ${hasFamilyHistory === true ? 'active yes' : ''}`} disabled={isLoading}>
                <span className="btn-icon">✅</span>
                <span className="btn-text">Yes</span>
              </button>
              <button type="button" onClick={() => { setHasFamilyHistory(false); setFamilyMembers([]); setError('') }}
                className={`yes-no-btn ${hasFamilyHistory === false ? 'active no' : ''}`} disabled={isLoading}>
                <span className="btn-icon">❌</span>
            ...
            <div className="family-member-form">
              <div className="form-row">
                <div className="form-group">
                  ...
                  <select id="relation" value={currentMember.relation}
                    onChange={(e) => setCurrentMember({ ...currentMember, relation: e.target.value })}
                    disabled={isLoading} className="relation-select">
                ...
                <div className="form-group">
                  ...
                  <input id="age-diagnosis" type="number" min="0" max="150"
                    value={currentMember.ageAtDiagnosis}
                    onChange={(e) => setCurrentMember({ ...currentMember, ageAtDiagnosis: e.target.value })}
                    placeholder="e.g., 65" disabled={isLoading} className="age-input" />
                </div>
                <button type="button" onClick={handleAddFamilyMember} disabled={isLoading} className="btn btn-secondary btn-add">
                  ➕ Add
                </button>
              </div>
            </div>
                ...
                <div className="members-grid">
                  {familyMembers.map((member) => (
                    <div key={member.id} className="member-card">
                      ...
                      <button type="button" onClick={() => handleRemoveFamilyMember(member.id)}
                        disabled={isLoading} className="btn-remove">✕</button>
                    </div>
                  ))}
                </div>
              </div>
