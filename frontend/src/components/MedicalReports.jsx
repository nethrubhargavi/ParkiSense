...
/* ──── API helper ──── */
async function analyzeReports(reportType, files) {
  const form = new FormData();
  files.forEach((f) => form.append("files", f.file));

  const res = await fetch(
    `${API_BASE}/reports/analyze?report_type=${reportType}`,
    {
      method: "POST",
      body: form,
    }
  ...
  if (!res.ok) {
    throw new Error(data?.detail || "Server error while analysing reports");
  }
...
const REPORT_CATEGORIES = [
  {
    ...
    accent: "#e53e3e",
    description:
      "CBC, metabolic panel, lipid profile, liver & thyroid",
  ...
  },
  {
    ...
    accent: "#38a169",
    description:
      "Photoplethysmography, heart rate, SpO2 reports",
    accept: ".pdf,.jpg,.jpeg,.png",
  },
  {
    id: "general",
    ...
    accent: "#805ad5",
    description:
      "Prescriptions, clinical notes, discharge summaries",
    accept: ".pdf,.jpg,.jpeg,.png",
  },
];
...
export default function MedicalReports() {
  const [sections, setSections] = useState(() =>
    Object.fromEntries(
      REPORT_CATEGORIES.map((c) => [c.id, emptyState()])
    )
  );
  ...
  function addFiles(catId, fileList) {
    ...
    update(catId, {
      files: [...sections[catId].files, ...newFiles],
    });
  }
  ...
  async function handleAnalyze(catId) {
    ...
    if (!section.files.length) {
      update(catId, { error: "Please upload at least one report." });
      return;
    }
    ...
    try {
      const data = await analyzeReports(catId, section.files);
      ...
      setTimeout(
        () => update(catId, { loading: false, progress: 0 }),
        600
      );
    }
  }
                    ...
                    <div className="cards">
                      ...
                      <div className="card">
                        ...
                        {s.result.analysis
                          ?.keyFindings?.length ? (
                          <ul>
                            {s.result.analysis.keyFindings.map(
                              (kf, i) => (
                                <li key={i}>{kf}</li>
                              )
                            )}
                      ...
                      <div className="card">
                        ...
                        {s.result.analysis
                          ...
                          ?.length ? (
                          <ul>
                            {s.result.analysis.abnormalIndicators.map(
                              (ai, i) => (
                                <li key={i}>
                                  <strong>
                                    {ai.values.join(
                      ...
                      <div className="card">
                        ...
                        <ul>
                          {s.result.analysis?.healthInsights?.map(
                            (hi, i) => (
                              <li key={i}>{hi}</li>
                            )
                          )}
