...
export default function MedicalReports() {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const res = await apiFetch(`${API_BASE}/reports/analyze`, {
        method: 'POST',
        body: form
      })
  ...
  return (
    <div className="reports-container">
      ...
      <div ref={dropRef} className="drop-area" onDrop={handleDrop} onDragOver={handleDragOver}>
        <p>Drag & drop files here, or click to select</p>
        <input type="file" multiple accept=".pdf,image/png,image/jpeg"
      ...
      <div className="actions">
        <button className="btn btn-primary" onClick={handleAnalyze} disabled={files.length === 0 || loading}>
          ...
          <div className="cards">
            ...
            <div className="card">
              ...
              {result.analysis?.keyFindings?.length ? (
                <ul>{result.analysis.keyFindings.map((kf, i) => <li key={i}>{kf}</li>)}</ul>
            ...
            <div className="card">
              ...
              <ul>{result.analysis?.healthInsights?.map((hi, i) => <li key={i}>{hi}</li>)}</ul>
            </div>
          </div>
        )}
      </div>
